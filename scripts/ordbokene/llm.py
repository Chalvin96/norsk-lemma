...
# Conservative ARG_MAX headroom: prompts above this are rejected for arg-input
# harnesses (opencode/pi) rather than overflowing into an opaque OSError.
...
def _codex_argv(config: LlmConfig, output_path, prompt_path) -> list[str]:
    effort = ["-c", f"model_reasoning_effort={config.reasoning_effort}"] if config.reasoning_effort else []
    return [
        "codex", "exec", "--model", config.model, *effort,
...
def _claude_argv(config: LlmConfig, output_path, prompt_path) -> list[str]:
    ...
    return ["claude", "-p", "--model", config.model, "--output-format", "text",
            "--disallowedTools", "*"]
...
def _pi_argv(config: LlmConfig, output_path, prompt_path) -> list[str]:
    ...
    return [
        "pi", "-p", "--mode", "text", "--no-tools", "--no-session", "--no-context-files",
        "--model", config.model, *thinking,
    ]


HARNESSES: dict[str, CliHarness] = {
    "codex": CliHarness("codex", _codex_argv, input_mode=InputMode.STDIN, output_mode=OutputMode.FILE),
    "claude": CliHarness("claude", _claude_argv, input_mode=InputMode.STDIN, output_mode=OutputMode.STDOUT),
    "opencode": CliHarness("opencode", _opencode_argv, input_mode=InputMode.ARG, output_mode=OutputMode.STDOUT),
    "droid": CliHarness("droid", _droid_argv, input_mode=InputMode.FILE, output_mode=OutputMode.STDOUT),
...
def complete_cli(harness: CliHarness, config: LlmConfig, prompt: str) -> str:
    ...
    """
    if harness.input_mode == InputMode.ARG and len(prompt.encode("utf-8")) > _MAX_ARG_PROMPT_BYTES:
        raise LlmError(
            f"{harness.name}_prompt_too_large",
            f"{len(prompt)} chars exceeds arg limit; lower --batch-size",
        )
    ...
    for attempt in range(1, config.max_retries + 1):
        ...
        with tempfile.TemporaryDirectory() as workdir:
            ...
            run_kwargs: dict[str, Any] = dict(
                text=True, capture_output=True, cwd=workdir, timeout=CLI_TIMEOUT, check=False
            )
            ...
            if completed.returncode != 0:
                ...
                logger.warning(
                    ...
                    harness.name, completed.returncode, attempt, config.max_retries, message[:200],
                )
                time.sleep(config.retry_delay)
                continue
