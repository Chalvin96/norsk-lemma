...
def cmd_hydrate(args: argparse.Namespace) -> None:
    ...
    for article_id, raw, lemma_path in tqdm(pending, unit="article", desc="Hydrating"):
        ...
        source_ids_by_text: dict[str, list[int]] = {}
        for definition in article_defs:
            source_ids_by_text.setdefault(definition["text"], []).append(definition["source_id"])
        synthetic_defs = []
        for definition in lemma_data.get("definitions", []):
            text = definition.get("text")
            source_ids = source_ids_by_text.get(text, [])
            if not source_ids:
                continue
...
def _collect_untranslated(exploded: list[ExplodedEntry], force: bool) -> list[ExplodedEntry]:
    if force:
        return exploded
    return [(aid, raw) for aid, raw in exploded if extract_existing_translations(raw) is None]
