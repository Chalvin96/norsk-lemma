...
import nb_g2p
from ordbokene.io import write_text_atomically
...
NOFABET_TO_IPA_MAP = {
    ...
    "J": "j",
    "V": "ʋ",
    "W": "w",  # V = ʋ (labio-dental approximant)
    "NG": "ŋ",
    "K": "k",
    "P": "p",
    "T": "t",
...
def g2p_entries(wordforms: list[str]) -> dict[str, PronEntry]:
    ...
    try:
        ...
        for input_word, (_result_word, nofabet_tokens) in zip(
            unique_wordforms, results, strict=False
        ):
...
def write_article_data(article_path: Path, data: dict) -> None:
    write_text_atomically(article_path, json.dumps(data, ensure_ascii=False))
...
def run(
    ...
    *,
    ...
    """Enrich article JSONs using the pronunciation indexes and G2P fallback."""
    for index in (primary_index, fallback_index, newwords_index, newwords_fallback_index):
        index.clear()
    if not leksika_path.exists():
        sys.exit(
            f"leksika not found: {leksika_path}\nSee data/README.md for download instructions."
        )
    if not newwords_path.exists():
        sys.exit(
            f"newwords not found: {newwords_path}\nSee data/README.md for download instructions."
        )
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = {pool.submit(enrich_article, p, force): p for p in articles}
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--leksika",
        default="data/nb_uttale_leksika/nb_uttale_leksika/e_written_pronunciation_lexicon.csv",
    )
    parser.add_argument(
        "--newwords", default="data/nb_uttale_tillegg/nb_uttale_tillegg/newwords_2022.csv"
    )
