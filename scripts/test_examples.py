...
def test_extract_existing_translations_caps_examples_to_max() -> None:
    from ordbokene.extract import extract_existing_translations
    from ordbokene.settings import MAX_EXAMPLES
    ...
    for i in range(MAX_EXAMPLES + 2):
        elements.append(
            {"type_": "example", "quote": {"content": f"ex{i}"}, "en": f"en{i}"}
        )
    raw = {
        "article_id": 1,
        "lemmas": [{"id": 1, "lemma": "gå", "primary_translation": "walk"}],
    ...
    result = extract_existing_translations(raw)
    assert result is not None
    assert len(result["definitions"][0]["examples"]) == MAX_EXAMPLES
...
def test_select_articles_force_includes_already_translated(tmp_path: Path) -> None:
    ...
    article = _make_article(examples=[
...
def test_embed_example_translations_sets_en_on_raw_refs() -> None:
    ...
    cores = [c for c in article["body"]["definitions"][0]["elements"] if c.get("type_") == "example"]
...
def test_run_dry_run_does_not_write(tmp_path: Path) -> None:
    ...
    examples = [e for e in data["body"]["definitions"][0]["elements"] if e.get("type_") == "example"]
...
def test_is_quota_error_classifies_quota_and_non_quota_messages() -> None:
    from ordbokene.examples import _is_quota_error

    assert _is_quota_error("codex_quota: You've hit your usage limit")
    assert _is_quota_error("http_429: Too Many Requests")
    assert _is_quota_error("Error 402: payment required")
    assert _is_quota_error("insufficient credit")
    assert not _is_quota_error("codex_exit_1: model not found")
    assert not _is_quota_error("json_parse_failed")
