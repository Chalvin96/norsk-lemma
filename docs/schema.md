...
Audio metadata is embedded into the article cache by the audio stage and copied
into the exported lemma payload by `export`. MP3 files and manifests live under
`data/export/audio/`.
...
- Each definition includes all source examples.
...
| `lemmas[].pos` | string | Canonical word class, `EXPR` for an expression, or `UNKNOWN` when no class is recognized. |
| `lemmas[].primary_translation` | string or null | Short English headword label. Set from the translate step's `lemma_primary` hook; when that is empty it falls back to the first non-blank definition gloss (see [primary_translation derivation](#primary_translation-derivation)). `null` when no headword meaning or definition gloss is available, including pure redirects. Expressions and genuine sub-articles retain their English meanings like main articles. |
...
| `definitions[].examples` | array | All source `{ "no", "en" }` example translation pairs. |

Articles whose source has no lemma are omitted because they cannot produce a
usable `lemmas[]` entry. Re-exporting removes any stale JSON file left by an
older source snapshot when that article no longer has a lemma.

## Word-form tag cleaning

`word_forms[].tags_json` carries morphology and source metadata. Ordinary word-class tags are removed; the `EXPR` marker is retained for compatibility. This section documents what Ordbokene emits and how `build_lemma` cleans it.

### Source tags (as emitted by Ordbokene)

Each paradigm carries its word-class tag in `paradigm_info.tags`, and each inflection row carries morphology in `inflection.tags`. The two are mixed into one tag list during build, so cleaning is what separates them.

**Word-class tags** (`paradigm_info.tags`) use the canonical set `NOUN VERB ADJ ADV PREP PRON CONJ INTERJ DET NUM` plus Universal Dependencies variants that Ordbokene emits in place of the canonical form:

| UD variant | Canonical | Meaning |
| --- | --- | --- |
| `ADP` | `PREP` | adposition (preposition in Norwegian) |
| `CCONJ` / `SCONJ` | `CONJ` | coordinating / subordinating conjunction |
| `INTJ` | `INTERJ` | interjection |
| `PROPN` | `NOUN` | proper noun |
| `SYM` | — | symbol (no canonical POS; falls back to `UNKNOWN`) |
| `X` | — | other (rare; falls back to `UNKNOWN`) |

**Morphology tags** (`inflection.tags`) are grammatical features. Ordbokene's vocabulary, observed across the export:

| Feature | Tags |
| --- | --- |
| Gender | `Masc`, `Fem`, `Neuter`, `Masc/Fem` (+ source variants `mask`/`fem`/`nøyt`/`Nøyt`) |
| Number | `Sing`, `Plur` |
| Definiteness | `Ind`, `Def` |
| Degree | `Pos`, `Cmp`, `Sup` |
| Verb form | `Inf`, `Pres`, `Past`, `Imp`, `Pass`, `<PerfPart>`, `<PresPart>`, `<SPass>` |
| Verb class | `v1`, `v2`, `v3`, `v4` (+ a redundant `verb` echo, often bundled as `verb, v2`) |
| Noun declension class | `m1`–`m3`, `n1`–`n3`, `f1`, `m.`, `n.`, `f.` (+ combinations like `m1, f1`) |
| Adjective class | `a1`–`a5` (+ combinations) |
| Pronoun / determiner | `Nom`, `Acc`, `Poss`, `Prs`, `Hum`, `Dem`, `Quant`, `Int`, `Refl`, `Rcp`, `Intens`, `Ord` (person is `1`/`2`/`3`) |
| Other / metadata | `Uninfl`, `EXPR`, `ABBR`, `PFX`, `COMPPFX`, `prop`, `i sms.`, `<ADJ>`, `<ADV>` |

### Cleaning steps

`build_lemma` (`scripts/ordbokene/build.py` `_source_morphology_tags`) strips ordinary word-class tags before export so the POS lives on `lemmas[].pos`. The `EXPR` compatibility marker is retained in word forms. Steps, in order:

1. **Normalize UD aliases** (`UD_TAG_ALIASES`): `ADP→PREP`, `CCONJ→CONJ`, `SCONJ→CONJ`, `INTJ→INTERJ`, `PROPN→NOUN`. Applied first so the POS detector (line 99) and the morphology cleaner agree on a canonical tag.
2. **Drop POS tags.** After aliasing, drop anything in `KNOWN_POS` (the canonical POS set) or the residual Universal Dependencies UPOS set `UD_UPOS` (`ADJ ADP ADV AUX CCONJ DET INTJ NOUN NUM PART PRON PROPN PUNCT SCONJ SYM VERB X`). This is what prevents opaque labels like `ADP`/`SYM`/`INTJ` leaking into the inflection table.
3. **Map gender variants** (`MORPHOLOGY_TAGS`): `nøyt→Neuter`, `mask→Masc`, `fem→Fem`, …

The redundant `verb` echo and Ordbokene declension-class markers (`v1`–`v4`, `m1`–`m3`, `a1`–`a5`, …) are **not** stripped here. `verb` is frequently bundled into a compound tag string (`"verb, v2"`) that also carries the verb-class signal, and bare `verb` always co-occurs with a tense tag, so removing it at the data layer would risk losing information for no display benefit. They stay out of the UI regardless: the consuming app's structured inflection tables read only the tags they need and ignore the rest (see the norsk lexicon domain doc).

After cleaning, `tags_json` contains the morphology/feature tags and declension-class markers from the table above. Ordinary POS tags are removed; `EXPR` remains as a compatibility marker.

### Non-inflecting lemmas keep a self-form

Prepositions, conjunctions, interjections, symbols, and numerals have no inflection rows, so `build_lemma` adds the lemma itself as a word form to carry base pronunciation. After cleaning, this self-form can have `tags_json: []`. It must be kept: the importer (`backend/scripts/ordbokene_importer.py` `_lemma_base_pron`) resolves the lemma's IPA from the word form whose `word_form == lemma`, and for a non-inflecting lemma that self-form is the sole carrier. An empty `tags_json` is valid and imports cleanly.

## primary_translation derivation

`primary_translation` is the short English label for the whole headword. `build_lemma` sets it in priority order:

1. The translate step's `lemma_primary` hook, when non-empty — a model-written umbrella over all senses (`"fish"`, `"pour / guzzle"`).
2. **Fallback:** the first non-blank `definitions[].translation`, when `lemma_primary` came back empty. For a single-sense article this gloss *is* the headword label; for multi-sense it is the leading sense, which `lemma_primary` would normally summarise.
3. `null` — a lemma with neither a headword meaning nor any definition gloss to derive from, including pure redirects.

Expressions export `pos: "EXPR"` and retain a definition-based
`primary_translation`. Their word forms keep the `EXPR` marker in `tags_json`
for compatibility. Expression entries and genuine sub-articles carry
`is_sub_article: true`; that flag alone does not distinguish the two.
A genuine sub-article such as `hygge seg` can therefore carry a normal
`primary_translation` (`"enjoy oneself"`) without being an expression.

The fallback keeps translated lemmas importable without extra model calls.
A `null` on a non-redirect lemma that carries definition glosses is a defect,
including when its POS is `EXPR`.

## Audio field reference

Audio metadata is written into the article cache by the audio step and copied
into `data/export/lemma/` by `export`. MP3s and manifests live under
`data/export/audio/`.
