...
- Each definition includes at most four examples.
...
| `definitions[].examples` | array | Up to four `{ "no", "en" }` example translation pairs. |

## Word-form tag cleaning

`word_forms[].tags_json` carries **morphology only**, never the word-class tag. This section documents what Ordbokene emits and how `build_lemma` cleans it.
...
`build_lemma` (`scripts/ordbokene/build.py` `_source_morphology_tags`) strips the word-class tag before export so the POS lives on `lemmas[].pos` exactly once and is not duplicated as a morphology tag. Steps, in order:
...
After cleaning, `tags_json` contains the morphology/feature tags and declension-class markers from the table above; POS tags never appear.
...
3. `null` — an `EXPR`-tagged expression (`build_lemma` forces `primary` null for these regardless of hook or gloss, since no single-lemma label fits an idiom), or a lemma with neither a `lemma_primary` hook nor any definition gloss to derive from.

Genuine sub-articles are **not** forced null — only the lemma-level `EXPR` tag is (`is_expression = "EXPR" in tags`). A real sub-article such as `hygge seg` (`is_sub_article: true`) carries a normal `primary_translation` (`"enjoy oneself"`). In the export the `EXPR` flag folds into `is_sub_article`, but it is still recoverable: an expression's self-form keeps `EXPR` in its `word_forms[].tags_json`.

Without the fallback a lemma whose `lemma_primary` returned empty would export `primary_translation: null` and then be **skipped on import**, silently dropping the headword from the backend. The fallback keeps those lemmas importable with no extra model calls. A `null` on a non-`EXPR` lemma that *does* carry definition glosses is therefore a defect, not an expected state.
