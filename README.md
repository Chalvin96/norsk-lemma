...
| `translate` | Generate headword glosses, sense glosses, and example translations via an LLM. |
...
uv run python pipeline.py translate --limit 20
...
and example translations, matching definitions by Norwegian text and examples
by position.
Use a compatible article snapshot: changed example order is not realigned
automatically. Hydration does not install MP3s or restore audio metadata into
...
Definitions contain Norwegian `text`, English `translation`, and up to
**four** `{ "no", "en" }` example pairs, controlled by `MAX_EXAMPLES`.
Missing English examples use an empty string. Some expression or
cross-reference entries have no headword gloss. Frequency and audio fields
are enrichments and may be missing in intermediate exports.

The [schema reference](docs/schema.md) describes both JSON layers, the
four-example cap, and the article-first audio workflow. Use
